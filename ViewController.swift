...
let whiteBgColor: UIColor = UIColor(red: 254.0/255.0, green: 254.0/255.0, blue: 254.0/255.0, alpha: 0.90)
let disabledWhiteBgColor: UIColor = UIColor(red: 254.0/255.0, green: 254.0/255.0, blue: 254.0/255.0, alpha: 0.10)

//Accesory View buttons tags
let deleteWaypointButtonTag = 666
...
class ViewController: UIViewController, UIGestureRecognizerDelegate  {
        ...
        didSet {
            if hasWaypoints {
              followUserButton.setImage(UIImage(named: "follow_user"), for: UIControlState())
                 ...
                 saveButton.setImage(UIImage(named: "saveHigh"), for: UIControlState())
                 resetButton.setImage(UIImage(named: "resetHigh"), for: UIControlState())
    ...
    var gpxTrackingStatus: GpxTrackingStatus = GpxTrackingStatus.notStarted {
        didSet {
            ...
            case .notStarted:
                ...
                trackerButton.setImage(UIImage(named: "start"), for: UIControlState())
            ...
            case .tracking:
                ...
                trackerButton.setImage(UIImage(named: "pause"), for: UIControlState())
                resetButton.setImage(UIImage(named: "resetHigh"), for: UIControlState())
                saveButton.setImage(UIImage(named: "saveHigh"), for: UIControlState())
            ...
            case .paused:
               ...
               // self.trackerButton.setTitle("Resume", for: UIControlState())
                 trackerButton.setImage(UIImage(named: "resume"), for: UIControlState())
    ...
    override func viewDidLoad() {
        ...
        let mapH: CGFloat = self.view.bounds.size.height - 70.0
        ...
        // HEADER
        let font36 = UIFont(name: "AppleSDGothicNeo-Bold", size: 36.0)
        let font18 = UIFont(name: "AppleSDGothicNeo-Bold", size: 18.0)
        let font16 = UIFont(name: "AppleSDGothicNeo-Bold", size: 16.0)
        ...
        //appTitleLabel.text = "Track my Walk" vvv ccc   ccfc
        //appTitleLabel.textAlignment = .left
        //appTitleLabel.font = UIFont.boldSystemFont(ofSize: 18)
        
        //appTitleLabel.textColor = UIColor(red: 0.0/255.0, green: 0.0/255.0, blue: 0.0/255.0, alpha: 1.0)
        //appTitleLabel.textColor = UIColor.red
        ...
        // CoordLabel
        coordsLabel.frame = CGRect(x: self.map.frame.width - 305, y: 20, width: 300, height: 16)
        coordsLabel.textAlignment = .right
        coordsLabel.font = font16
        coordsLabel.textColor = UIColor.gray
        coordsLabel.text = "GPS ,Unable to locate!"
        ...
        folderButton.backgroundColor = whiteBgColor
        ...
        //trackerButton.setTitle("Start Tracking", for: UIControlState())
        //trackerButton.backgroundColor = whiteBgColor
        trackerButton.setImage(UIImage(named: "start"), for: UIControlState())
        trackerButton.setImage(UIImage(named: "start"), for: .highlighted)
        ...
        //newPinButton.backgroundColor = whiteBgColor
        ...
        //followUserButton.backgroundColor = whiteBgColor
        ...
        //saveButton.backgroundColor = disabledWhiteBgColor
        saveButton.setImage(UIImage(named: "save"), for: UIControlState())
       ...
       // resetButton.backgroundColor = disabledWhiteBgColor
        resetButton.setImage(UIImage(named: "reset"), for: UIControlState())
        resetButton.setImage(UIImage(named: "resetHigh"), for: .highlighted)
    ...
    func openFolderViewController() {
        ...
        let vc = GPXListViewController(nibName: nil, bundle: nil)
        vc.delegate = self
   ...
   func addPinAtTappedLocation(_ gesture: UILongPressGestureRecognizer) {
    ...
    myLocationCoordinate = map.userLocation.coordinate
    var fromLocation: CLLocation = CLLocation(latitude: (myLocationCoordinate?.latitude)!, longitude: (myLocationCoordinate?.longitude)!)
    
  

       if  gesture.state == UIGestureRecognizerState.began {
            print("Adding Pin map Long Press Gesture")
           // let touchPoint: CGPoint = gesture.location(in: self.map)
          touchPoint = gesture.location(in: self.map)
        newCoordinate = map.convert(touchPoint!,toCoordinateFrom: self.map)
    
        let waypoint = GPXWaypoint(coordinate: newCoordinate!)
        

        map.addWaypoint(waypoint)
      
        var toLocation: CLLocation
        = CLLocation(latitude: (newCoordinate?.latitude)!, longitude: (newCoordinate?.longitude)!)
        var distance: CLLocationDistance  = toLocation.distance(from: fromLocation)
        
        let formatted = String(format: "%.2f", (Double(distance) / 1609))
        waypoint.title = "Distance : \(formatted) miles"
     
        
        self.hasWaypoints = true
        
        
         // map.addWaypointAtViewPoint(touchPoint!)
        
            //Allows save and reset
          self.hasWaypoints = true
        
    
        //var touchPoint = gesture.location(in: self.map)
        
        
        /*var newCoordinate:CLLocationCoordinate2D = map.convert(touchPoint,toCoordinateFrom: self.map)*/
        
        print("\(newCoordinate?.latitude) and \(newCoordinate?.longitude)")
    ...
    func addPinAtMyLocation() {
        ...
        print(" This is the current location \(map.userLocation.coordinate.latitude) and \(map.userLocation.coordinate.longitude)")
        map.addWaypoint(waypoint)
        waypoint.title = "This is the title"
        self.hasWaypoints = true
        
        // source and destination are the relevant MKMapItems
        //var destinationCoordinates: CLLocationCoordinate2D = newCoordinate
        if( newCoordinate != nil){
            let request:MKDirectionsRequest = MKDirectionsRequest()
            
            request.source = MKMapItem(placemark: MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: (newCoordinate?.latitude)!, longitude: (newCoordinate?.longitude)!), addressDictionary: nil))
            request.destination = MKMapItem(placemark: MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: map.userLocation.coordinate.latitude, longitude: map.userLocation.coordinate.longitude), addressDictionary: nil))
            ...
            directions.calculate { [unowned self] response, error in
                ...
                for route in unwrappedResponse.routes {
                    ...
                    self.map.setVisibleMapRect(route.polyline.boundingMapRect, animated: false)
                }
    ...
    func saveButtonTapped() {
        ...
        let alert = UIAlertView(title: "Save as", message: "Log workout activity name", delegate: self, cancelButtonTitle: "Continue working out")
...
extension ViewController: GPXListViewControllerDelegate {
